  ...
  bindRouter: function bindRouter(pages) {
    ...
    while (!!pages.length) {
      ...
      options[routeId] = _.bind(function (page) {
        ...
        if (page.url.startsWith('*')) {
          ...
          try {
            for (var _iterator = page.url.split('/')[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
              ...
              if (part.startsWith(':')) {
                params[part.substr(1)] = arguments.length <= idx++ + 1 ? undefined : arguments[idx++ + 1];
              }
            }
        ...
        var queryStr = arguments.length <= (arguments.length <= 1 ? 0 : arguments.length - 1) - 1 + 1 ? undefined : arguments[(arguments.length <= 1 ? 0 : arguments.length - 1) - 1 + 1];
