...
function validator(key, value) {
  if (this[REQUIRED] && !partialValidator[REQUIRED].call(this, key, value)) {
    return Error.create(`Required attribute "${key}" is missing.`);
  }

  if (this[MIN] && !partialValidator[MIN].call(this, key, value)) {
    if (this.name === 'number') {
      return Error.create(`"${key}" is greater than ${this[MIN]}.`);
    } else {
      return Error.create(`"${key}" must be more than ${this[MIN]}${this[MIN]} characters long.`);
    }
  }
}
