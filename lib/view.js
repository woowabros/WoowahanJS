...
View = Backbone.View.extend({
      ...
      if (!!childMatch) {
        ...
        listener = function (eventName, selector, method, params, event) {
          ...
          var values = params.map(function (param) {
            return getVal(_this.querySelector(param));
          });
          ...
          if (eventName === 'submit') {
            ...
            try {
              for (var _iterator = _this.querySelector(selector).querySelectorAll('input, select, textarea')[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
                var el = _step.value;
  ...
  updateView: function updateView(container, ChildView) {
    ...
    var viewContainer = typeof container === 'string' ? this.el.querySelector(container) : container;
