...
View = Backbone.View.extend({
  ...
  _bindRef: function _bindRef() {
    ...
    _.each(this.$el.find('[data-ref-name]'), _.bind(function (element) {
      var refName = element.dataset.refName;
      var refGroup = element.dataset.refGroup || 'false';
      var refRestore = element.dataset.refRestore || 'false';

      if (refGroup.toLowerCase() === 'true') {
        ...
        if (currentElement) {
          refRestore.toLowerCase() === 'true' && this._syncElement(currentElement, this.refs[refName]);
          currentElement = null;
        }
      }
    }, this));
