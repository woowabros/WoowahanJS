  ...
  super() {
    ...
    if (!!this.model) {
      if (typeof this.model.toJSON === 'function') {
        ...
        renderData = _.clone(this.model);
      }
    }
    
    if (typeof this.beforeMount === 'function') {
      renderData = this.beforeMount(renderData) || renderData;
    }
    
    this.trigger('beforeMount');
    
    if (!this.template) {
      console.warn('view는 template를 가져야 합니다. ( 필수 )');
    }
    
    if (typeof this.template === 'string') {
      domStr = this.template;
    } else {
      const tagName = this.tagName || 'div';
      
      domStr = `<${tagName}>${this.template(renderData)}</${tagName}>`;
    }
