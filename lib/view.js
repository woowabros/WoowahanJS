...
viewMount = function viewMount() {
  var middlewares = app.getMiddleware(MIDDLEWARE.VIEW, MIDDLEWARE_PROTOCOL.BEFORE);
...
View = Backbone.View.extend({
  ...
  updateView: function updateView(container, ChildView) {
    ...
    if (!!view) {
      ...
      var middlewares = app.getMiddleware(MIDDLEWARE.VIEW, MIDDLEWARE_PROTOCOL.UNMOUNT);
  ...
  _bindModel: function _bindModel() {
    ...
    try {
      for (var _iterator3 = targetElements[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
        ...
        var _value2 = this.model.get(key);
        ...
        if (typeof _value2 !== 'undefined') this._plugins[type].call(this, element, _value2);
      }
    } catch (err) {
    ...
    try {
      for (var _iterator4 = targetElements[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
        ...
        plugins.split('+').map(function (s) {
          ...
          var _plugin$split$map2 = _slicedToArray(_plugin$split$map, 2);

          var keys = _plugin$split$map2[0];
          var type = _plugin$split$map2[1];
          ...
          keys.forEach(function (key) {
            ...
            var value = _this3.model.get(key);

            _this3.listenTo(_this3.model, 'change:' + key, function (element, key, type) {
              var value = this.model.get(key);

              this._plugins[type].call(this, element, value);
            }.bind(_this3, element, key, type));
          });

          if (typeof value !== 'undefined') _this3._plugins[type].call(_this3, element, value);
        });
      }
    } catch (err) {
